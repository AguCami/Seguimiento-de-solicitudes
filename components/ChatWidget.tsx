...
export function ChatWidget({ myId }: { myId: string }) {
  ...
  const fetchMessages = useCallback(async (userId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setTotalUnread(prev => Math.max(0, prev - (conversations.find(c => c.user.id === userId)?.unread ?? 0)));
      }
    } catch {}
  }, [conversations]);
  ...
  useEffect(() => {
    ...
    if (activeUser && view === "chat") {
      pollRef.current = setInterval(() => fetchMessages(activeUser.id), 4000);
    }
    return () => { if (pollRef.current) clearInterval(pollRef.current); };
  }, [activeUser, view, fetchMessages]);
  ...
  useEffect(() => {
    if (view === "chat" && open) setTimeout(() => inputRef.current?.focus(), 100);
  }, [view, open]);
      ...
      {open && (
        <div style={panelStyle}>
          ...
          <div style={{
            ...
            {view === "chat" && (
              <button onClick={() => { setView("list"); setActiveUser(null); }} style={{
                background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)",
                borderRadius: "8px", padding: "4px 8px", color: "rgba(255,255,255,0.7)",
            ...
            {view === "chat" && activeUser && (
              <span style={{
                ...
                background: "rgba(255,255,255,0.08)", borderRadius: "999px", padding: "2px 8px",
              }}>{roleLabel[activeUser.role] ?? activeUser.role}</span>
            )}
          </div>
          ...
          {view === "list" && (
            <div style={{ flex: 1, overflowY: "auto" }}>
              {conversations.length === 0 && (
                <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "13px", textAlign: "center", padding: "40px 16px" }}>
              ...
              {conversations.map((conv) => (
                ...
                >
                  <div style={{
                    ...
                    background: "rgba(102,126,234,0.5)", border: "1px solid rgba(255,255,255,0.2)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: "15px", fontWeight: 700, color: "white",
                  }}>{conv.user.name[0].toUpperCase()}</div>
                  <div style={{ flex: 1, minWidth: 0, textAlign: "left" }}>
                    ...
                    <p style={{
                      fontSize: "12px", color: conv.unread > 0 ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.35)",
          ...
          {view === "chat" && (
            <>
              <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: "6px" }}>
                {messages.length === 0 && (
                  <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "12px", textAlign: "center", margin: "auto 0" }}>
              ...
              <form onSubmit={sendMessage} style={{
                ...
                <input
                  ...
                  style={{
                    ...
                    borderRadius: "12px", padding: "8px 12px", fontSize: "13px", color: "white",
                    outline: "none",
                  }}
                />
                <button type="submit" disabled={!input.trim() || sending} style={{
                  background: "rgba(102,126,234,0.8)", border: "1px solid rgba(102,126,234,0.9)",
